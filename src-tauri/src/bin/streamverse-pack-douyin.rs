    ...
    let owned_cookie_file = match (cookie_file, cookie_browser) {
        (Some(path), Some(browser)) => {
            ...
            if cookie_file_contains_login_cookie(
                ...
                Some(export_browser_cookies_for_url(browser, "https://www.douyin.com/")?)
            }
        }
...
fn run_profile_browser_action(
    ...
    let mut should_cleanup_cookie_file = false;
    let owned_cookie_file = match (cookie_file, cookie_browser) {
        (Some(path), Some(browser)) if action == "collect-profile-browser" => {
            let path_buf = PathBuf::from(path);
            if cookie_file_contains_login_cookie(
                &path_buf,
                &["douyin.com", "iesdouyin.com"],
        ...
        (None, Some(browser)) if action == "collect-profile-browser" => {
            should_cleanup_cookie_file = true;
            Some(export_browser_cookies_for_url(browser, "https://www.douyin.com/")?)
        }
        _ => None,
