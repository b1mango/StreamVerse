...
class ProgressTracker:
    ...
    def total(self) -> int:
        return max(self.current, self.regular_total + self.pending_group_estimate, 1)

    def emit(self, message: str) -> None:
        write_progress(self.current, self.total(), message)

    def finish(self) -> None:
        total = max(self.current, self.regular_total, 1)
...
async def main_async(args: argparse.Namespace) -> int:
    ...
    if not cookie_value:
        raise RuntimeError("请先在设置中选择已登录的浏览器 Cookie 后再读取 Bilibili 主页。")

    mid = extract_mid(args.url)
    tracker = ProgressTracker()
    write_progress(0, 1, "正在读取 Bilibili 主页视频…")
    headers = {
