        ...
        json.dumps(
            {
                ...
                "total": max(int(total), int(current)),
                "message": message,
            },
            ensure_ascii=False,
        ),
...
def cleanup_manual_browser_profile(path: Path) -> None:
...
def cleanup_stale_manual_profiles(max_age_seconds: int = 6 * 60 * 60) -> None:
    ...
    for child in root.iterdir():
        if not child.is_dir() or not child.name.startswith("manual-"):
            continue
        try:
        ...
        if modified_at <= deadline:
            cleanup_manual_browser_profile(child)
...
async def async_main(args: argparse.Namespace) -> int:
    ...
    manual_profile_dir = (
        browser_state_root() / f"manual-{browser_name}-{args.connect_port}"
    ...
    try:
        ...
        if manual_profile_dir is not None:
            cleanup_manual_browser_profile(manual_profile_dir)
