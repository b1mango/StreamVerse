...
- YouTube 批量清晰度补全增强：单条目失败自动退避重试（至多 3 次），工人错开启动以降低触发限流的概率；进度反馈即时化——条目被拾取即刷新「已完成 X/Y，进行中 Z」，进度条始终反映真实完成数。
- 深色主题背景由近黑（#0a0b0d）调整为 Cursor 默认深色色系（#1f1f1f 基准），暗色阶各层同步上移。
