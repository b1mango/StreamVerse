...
- 解析命中登录态失效类报错（如 YouTube Cookie 被服务端轮换）时，若该平台已开启「始终允许」授权，自动重取浏览器 Cookie 并重试一次，不再要求手动重新提权导入。
- 圆角语言收敛：`--radius-l` 收敛为 12px；无边框窗口增加内描边以界定窗口边缘（Win10 直角 / Win11 圆角均适用）；下载队列保持贴边方正形态。
- 设置：检查更新加载文案缩短以防溢出；并行下载数超界（1–8）时给出内联提示；登录态区分「自动获取 / 手动获取」分组并移除认证目录说明文字。
- 英文模式改用内嵌的 IBM Plex Sans 可变字体（与用户站点字体一致），中文模式不变。
- YouTube 批量清晰度补全的进度条在后台补全期间持续显示；合集解析进度文案与频道主页区分（「正在读取合集视频…」）。
...
- YouTube 主页/合集清晰度补全并发从 4 提升到 8。
- 标题栏版本号改为读取应用真实版本（如 1.0.0），移除无边框标题栏中的应用名文字；toast 文案统一去掉结尾句号；历史页平台标识不再强制大写。
- YouTube 登录态治理：关键 Cookie 校验放宽为 LOGIN_INFO 或 SAPISID 家族任一在场即可（SAPISIDHASH 鉴权）；本地 Cookie 文件校验未过时公开内容自动降级为来宾模式继续解析，不再整块阻断；「缺少关键 Cookie」类报错纳入登录态自愈路径（已授权浏览器静默重取并重试）。
- YouTube 批量清晰度补全增强：单条目失败自动退避重试（至多 3 次），工人错开启动以降低触发限流的概率；进度反馈即时化——条目被拾取即刷新，进度条始终反映真实完成数（已完成 X/Y）。
- 深色主题背景与个人站（b1mango.cc）一致：`#2e2838 → #2a222d → #08070a` 黑紫径向渐变叠加双层 feTurbulence 噪点纹理（overlay + soft-light），暗色阶各层改为紫调近黑；浅色主题不带噪点。
- 应用图标全套替换为新版「星环星球」：Windows exe 图标、NSIS 安装包图标、macOS icns、窗口左上品牌位与设置「关于」页 logo 同源更新。
